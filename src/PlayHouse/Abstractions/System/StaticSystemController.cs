...
/// ServerId 패턴을 기반으로 ServiceId를 자동 추론합니다:
/// - "play-*" -> ServiceType.Play (1)
/// - "api-*" -> ServiceType.Api (2)
/// - 그 외 -> 기본값 1
/// </remarks>
public sealed class StaticSystemController : ISystemController
{
    ...
    public Task<IReadOnlyList<IServerInfo>> UpdateServerInfoAsync(IServerInfo serverInfo)
    {
        ...
        foreach (var kvp in _peers)
        {
            ...
            var serviceId = InferServiceId(serverId);
            servers.Add(new XServerInfo(serviceId, serverId, address));
        }
    ...
    /// <summary>
    /// ServerId 패턴으로부터 ServiceId를 추론합니다.
    /// </summary>
    /// <param name="serverId">서버 ID.</param>
    /// <returns>추론된 ServiceId.</returns>
    private static ushort InferServiceId(string serverId)
    {
        if (serverId.StartsWith("play-", StringComparison.OrdinalIgnoreCase))
        {
            return (ushort)ServiceType.Play;
        }

        if (serverId.StartsWith("api-", StringComparison.OrdinalIgnoreCase))
        {
            return (ushort)ServiceType.Api;
        }
