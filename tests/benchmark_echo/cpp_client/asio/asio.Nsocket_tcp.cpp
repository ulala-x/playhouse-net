	...
	this->m_socket.async_read_some(m_receiving_msg,
		[this, self](boost::system::error_code _error_code, std::size_t _length)
		{
			...
			try
			{
				while (length >= 4)
				{
					// - get message body size (PlayHouse protocol: Length field = body size only)
					uint32_t body_size = *reinterpret_cast<uint32_t*>(pdata);

					// check)
					if (body_size == 0)
						throw std::length_error("message length is zero");

					// - calculate total message size (4-byte length prefix + body)
					message_size = 4 + body_size;
