...
using PlayHouse.Benchmark.Echo.Server;
...
var rootCommand = new RootCommand("PlayHouse Benchmark Echo Server")
{
    tcpPortOption,
    zmqPortOption,
    httpPortOption,
...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File(
        Path.Combine(logDir, "benchmark-echo-server-.log"),
...
{
    Log.Information("================================================================================");
    Log.Information("PlayHouse Benchmark Echo Server");
    Log.Information("================================================================================");
    ...
    services.AddPlayServer(options =>
    {
        ...
        options.ServerId = "echo-bench-1";
        options.BindEndpoint = $"tcp://0.0.0.0:{zmqPort}";
        options.TcpPort = tcpPort;
        options.AuthenticateMessageId = "AuthenticateRequest";
        options.DefaultStageType = "EchoStage";
        options.RequestTimeoutMs = 60000; // 60초 타임아웃 (대량 연결 시 Stage 생성 시간 고려)
    })
    .UseStage<EchoStage, EchoActor>("EchoStage")
    .UseSystemController<EchoSystemController>();
