...
public class BootstrapServerE2ETests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _playServer = new PlayServerBootstrap()
            .Configure(options =>
            {
                ...
                options.ClientEndpoint = "tcp://127.0.0.1:0"; // 자동 포트 할당
    ...
    public void PlayServerBootstrap_StartsSuccessfully()
    {
        ...
        _playServer.ClientPort.Should().BeGreaterThan(0, "클라이언트 포트가 할당되어야 함");
    ...
    [Fact(DisplayName = "RequestAsync - StageId와 함께 요청하면 응답에도 StageId가 포함된다")]
    public async Task RequestAsync_WithStageId_ResponseIncludesStageId()
    {
        // Given (전제조건): 서버에 연결된 상태
        await ConnectToServerAsync();
