...
public class ConnectorE2ETests : IAsyncLifetime
{
    ...
    {
        // Given (전제조건): 잘못된 포트로 설정
        _connector.Init(new ConnectorConfig
        {
            Host = "127.0.0.1",
            Port = 59999 // 존재하지 않는 포트
        });
    ...
    public async Task Disconnect_ByClient_DisconnectsWithoutCallback()
    {
        ...
        // 참고: 클라이언트가 직접 Disconnect()를 호출하면 OnDisconnect 콜백은 호출되지 않음
        // OnDisconnect는 서버 측에서 연결이 끊어졌을 때만 호출됨 (예: 서버 종료, 네트워크 오류)
        _connector.Disconnect();
        await Task.Delay(100);

        // Then (결과): 연결만 해제됨 (콜백은 호출되지 않음)
    ...
    public async Task OnDisconnect_ServerClosesConnection_InvokesCallback()
    {
        ...
        // 콜백 대기 (MainThreadAction을 반복 호출하며 대기)
    ...
    public async Task IsConnected_ReflectsConnectionState()
    {
        ...
        _connector.Init(new ConnectorConfig { Host = "127.0.0.1", Port = _server.Port });
    ...
    [Fact(DisplayName = "RequestAsync with StageId - StageId가 서버에 전달된다")]
    public async Task RequestAsync_WithStageId_StageIdIsSentToServer()
    {
        // Given (전제조건): 서버에 연결된 상태
        await ConnectToServerAsync();
        ...
        // When (행동): StageId와 함께 요청
        var response = await _connector.RequestAsync(testStageId, packet);
        await ProcessCallbacksAsync();
    ...
    public async Task Request_WithCallback_InvokesCallbackWithResponse()
    {
        ...
        // Then (결과): 콜백이 응답과 함께 호출됨 (MainThreadAction을 반복 호출하며 대기)
        var timeout = DateTime.UtcNow.AddSeconds(5);
    ...
    {
        // Given (전제조건): 서버에 연결되고 인증된 상태 (Send(IPacket)은 인증 필요)
        await ConnectToServerAsync();

        // 인증 수행
        _connector.SetAuthenticateMessageId("AuthenticateRequest");
        using var authPacket = Packet.Empty("AuthenticateRequest");
        await _connector.AuthenticateAsync(authPacket);

        _server.ReceivedMessages.Clear(); // 인증 메시지 제거

        var echoRequest = new EchoRequest { Content = "Fire and Forget", Sequence = 7 };
        using var packet = new Packet(echoRequest);

        // When (행동): 메시지 전송 (응답 기대 안함)
        _connector.Send(packet);
        await Task.Delay(100); // 전송 대기
    ...
    [Fact(DisplayName = "Send with StageId - StageId와 함께 메시지를 전송한다")]
    public async Task Send_WithStageId_ServerReceivesStageId()
    {
        // Given (전제조건): 서버에 연결된 상태
        await ConnectToServerAsync();
        ...
        // When (행동): StageId와 함께 전송
        _connector.Send(testStageId, packet);
        await Task.Delay(100);
    ...
    [Fact(DisplayName = "OnReceive - StageId가 포함된 Push 메시지를 수신한다")]
    public async Task OnReceive_PushWithStageId_ReceivesStageId()
    {
        // Given (전제조건): 서버에 연결된 상태 (이 테스트는 별도 Push 메시지 구현 필요)
        await ConnectToServerAsync();

        // Push 메시지의 stageId는 서버가 결정하므로,
        // 현재 구현에서는 0으로 전송됨
        // 실제 환경에서는 Stage별로 다른 stageId를 받게 됨
    }
    ...
    {
        // Given (전제조건): 서버에 연결되고 인증 메시지 ID 설정됨
        await ConnectToServerAsync();
        _connector.SetAuthenticateMessageId("AuthenticateRequest");
    ...
    public async Task Authenticate_WithCallback_InvokesCallbackOnSuccess()
    {
        ...
        // Then (결과): 인증 콜백 호출 (MainThreadAction을 반복 호출하며 대기)
        var timeout = DateTime.UtcNow.AddSeconds(5);
    ...
    public async Task RequestAsync_Timeout_ThrowsRequestTimeoutException()
    {
        ...
        // 서버가 응답하지 않는 메시지 (NoResponseRequest)
